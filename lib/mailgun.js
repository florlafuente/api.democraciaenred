...
function mailContacto (req, res, next) {
  if (!['name, email, reference, comments'].every(k => ~Object.keys(req.body).indexOf(k))) return res.status(400).json({ error: 'missing-params' })
  mg.validate(req.body.email, (error, validation) => {
    if (error) return next(error)
...
function mailTrabajo (req, res, next) {
  if (!['name', 'email', 'phone', 'comments', 'id'].every(k => ~Object.keys(req.body).indexOf(k))) return res.status(400).json({ error: 'missing-params' })
  if (!req.file) return res.status(400).json({ error: 'missing-cv' })
  mg.validate(req.body.email, (error, validation) => {
    if (error) return next(error)
